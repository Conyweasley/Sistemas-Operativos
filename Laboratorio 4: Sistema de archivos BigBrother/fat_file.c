                            ...
                            u32 nentry)
{
    ...
    u8 base_name[9]; // Need space for the null terminator
    u8 extension[4]; // Need space for the null terminator

    snprintf((char *)base_name, 9, "%.*s", 8, child_disk_entry->base_name);
    snprintf((char *)extension, 4, "%.*s", 3, child_disk_entry->extension);

    if (strcmp((char *)base_name, "fs") == 0 &&
        strcmp((char *)extension, "log") == 0)
    {
        child_disk_entry->base_name[0] = 0xe5;
...
static bool ignore_dentry(const fat_dir_entry disk_dentry)
{
    ...
    // char ext1 = disk_dentry->extension[0];
    // char ext2 = disk_dentry->extension[1];
    // char ext3 = disk_dentry->extension[2];
    // bool is_log_ext = (ext1 == 'l' && ext2 == 'o' && ext3 == 'g');
    // if (is_log_ext) {
    //    return false;
    //}
                                     ...
                                     fat_file dir, GList **elems)
{
         ...
         disk_dentry_ptr++, dir_entries_processed++)
    {
        ...
        u8 base_name = disk_dentry_ptr->base_name[0];
        u8 extension[4]; // Need space for the null terminator

        snprintf((char *)extension, 4, "%.*s", 3, disk_dentry_ptr->extension);
            ...
            strcmp((char *)extension, "log") == 0)
        {
            ...
            disk_dentry_ptr->attribs = FILE_ATTRIBUTE_ARCHIVE;
        }
...
// Implementacion de las funciones de bajo nivel para unlink
void fat_file_unlink(fat_file file, fat_file parent)
{
    // marcar como listo para eliminar
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;
    // esta funcion escribe la entrada de file en una posicion calculada con pos_in_parent en el file parent. Realiza la escritura en el disco de la nueva entrada.
    write_dir_entry(parent, file->dentry, file->pos_in_parent);
    // nuestro last_cluster es el primero de todos ya que queremos eliminar todo el file
